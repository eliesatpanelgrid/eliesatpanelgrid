...
# ============================================================
# UNIVERSAL MODEL DETECTION FOR ALL ENIGMA2 BOXES
# ============================================================
def get_box_model():
    """
    Detect box model for all Enigma2 devices
    Supports: Vu+, Dreambox, Zgemma, Octagon, Edision, Gigablue, Formuler, etc.
    """
    model = ""
    
    # Ordered list of paths to check (prioritize most accurate first)
    model_paths = [
        "/proc/stb/info/vumodel",      # Vu+ devices
        "/proc/stb/info/gbmodel",      # Gigablue devices
        "/proc/stb/info/boxtype",      # Zgemma, Octagon, Edision, etc.
        "/proc/stb/info/modelname",    # Some devices
        "/proc/stb/info/model",        # Generic
        "/proc/stb/info/boxbrand",     # Some devices
        "/etc/model",                  # Some images
        "/etc/boxmodel",               # Some images
        "/proc/device-tree/model",     # Some ARM devices
    ]
    
    for path in model_paths:
        if os.path.exists(path):
            val = safe_read(path)
            if val and val != "Unknown" and val != "" and val != "N/A" and val != "0":
                model = val
                break
    
    # If still empty, try with commands
    if not model or model == "" or model == "Unknown":
        commands = [
            "cat /proc/stb/info/vumodel 2>/dev/null",
            "cat /proc/stb/info/gbmodel 2>/dev/null",
            "cat /proc/stb/info/boxtype 2>/dev/null",
            "cat /proc/stb/info/modelname 2>/dev/null",
            "cat /proc/stb/info/model 2>/dev/null",
            "cat /etc/model 2>/dev/null",
            "grep -i 'model' /proc/cpuinfo 2>/dev/null | head -1 | cut -d':' -f2 | sed 's/^[ \t]*//'",
        ]
        for cmd in commands:
            result = run_cmd(cmd)
            if result and result != "Unknown" and result != "":
                model = result
                break
    ...
    if model:
        ...
        # model = model.lower()  # Uncomment if you want lowercase
    else:
        model = "Unknown"
...
"""
    ...
    def build_text(self):
        ...
        # Use universal model detection function
        model = get_box_model()
...
"""
    ...
    def detectStatus(self, html, reader):
        ...
        if reader["status"] == "OFF":
            ...
            block = re.search(r">" + re.escape(reader["label"]) + r"<.*?)<br/>", html, re.I | re.S)
            if not block:
                state = "Unknown"
                priority = 4
            else:
